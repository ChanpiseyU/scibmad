...
def disable(): 
    """Disable automatic PyTorch tensor handling for Julia functions."""
    global _original_call, _patched
    ...
    if _original_call is not None:
        juliacall.AnyValue.__call__ = _original_call
    _patched = False 

    # allows scibmad.enable() scibmad.disable(), remove auto enable so global behavior only changes when explicitly called + can be undone